...
%               d = d12 or [d12 d13 d23]
    ...
    function err = threeCircleAreaError (pos)
        ...
        %I(I==0) = Inf;
        I(I==0) = 1e3; % only very big, no infinite
        %Error
        %err = sum(abs((I-I0)./I0));
        % MY MOD %
        err = sum(abs((I(I0~=0) - I0(I0~=0))./I0(I0~=0)));
        err = err + sum(abs(I(I0==0))./eps);
    end
...
function hCirc = drawCirclesnorm(hParent, xc, yc, r, P, V, normalization)
    ...
    r = r(:);                   %Radii
    xc = xc .* sqrt(normalization  ./ sum(pi * r.^2));
    yc = yc .* sqrt(normalization  ./ sum(pi * r.^2));
    r = r .* sqrt(normalization ./ sum(pi * r.^2));
    n = length(r);              
