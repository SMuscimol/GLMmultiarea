...
DATADIR = '/burg/theory/users/spm2176/GDplusMemory/13-May-2021/';
...
tmpNovice = tmp(arrayfun(@(i) contains(tmp(i).name, '0_'), 1:numel(tmp)));
tmpExpert = tmp(arrayfun(@(i) contains(tmp(i).name, '1_'), 1:numel(tmp)));
% filesExpert = arrayfun(@(f) strcat(DATADIR, f.name), tmpExpert, 'UniformOutput', false)
% filesNovice = arrayfun(@(f) strcat(DATADIR, f.name), tmpNovice, 'UniformOutput', false)
