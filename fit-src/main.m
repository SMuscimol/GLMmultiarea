...
%%% This file fits a GLM encoding model to each neuron in the dataset, for a set of areas. It
%%% also fits several reduced models to run LR tests. By default, this
%%% is done for both Novice mice (MICEGROUP=0) and expert mice
%%% (MICEGROUP=1). You can change the variable micegroups to select only
%%% one of the two.
%%% If the <areas> variable is not set, it defaults to all the areas (not recommended).
...
%%% then setup the loop so to skip that neurons. 
%%% In the current version, only two neurons in the whole dataset were skipped.

%%% Set constants %%%
% WARNING: the environment variable WORK_HOME has to be set (you can do that using the function setenv)
WORK_HOME = getenv('WORK_HOME');  
testing = false;  % set to true to fit only neuron and verify the whole pipeline.
areas = {'ALM'};  % set this do a different variable to fit neurons in a different area!
if testing
    areas = {'wS1'};
else
...
dataDir = '<YOURDATADIR>'; % directory where the data is (adjust to your need!)
...
% After you fit all the areas, run collect_all_results.m (see instructions
% there)
%%% Samuel Pavio Muscinelli, 2020 %%%
